          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {p.category?.map((c, i) => <span key={i} className="rounded-full px-2.5 py-1 text-xs font-semibold" style={{ background: '#EEF2F6', color: 'var(--brand-navy-900)' }}>{c}</span>)}
            ...
            <div className="flex gap-3">
              {p.instagram_url && <a href={p.instagram_url} target="_blank" rel="noopener noreferrer" className="text-[#64748B] hover:text-[var(--brand-navy-900)]"><Instagram size={20} /></a>}
              {p.youtube_url && <a href={p.youtube_url} target="_blank" rel="noopener noreferrer" className="text-[#64748B] hover:text-[var(--brand-navy-900)]"><Youtube size={20} /></a>}
              {p.linkedin_url && <a href={p.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-[#64748B] hover:text-[var(--brand-navy-900)]"><Linkedin size={20} /></a>}
              {p.website_url && <a href={p.website_url} target="_blank" rel="noopener noreferrer" className="text-[#64748B] hover:text-[var(--brand-navy-900)]"><ExternalLink size={20} /></a>}
            </div>
          </div>
          ...
          <div className="mt-8 space-y-6">
            ...
            {p.lessons?.length > 0 && (
              <div>
                ...
                <ul className="space-y-2">{p.lessons.map((l, i) => <li key={i} className="flex gap-2 text-sm text-[#334155]"><span style={{ color: 'var(--brand-navy-900)' }}>·</span>{l}</li>)}</ul>
              </div>
            )}
            {p.student_takeaways && (
              <div className="rounded-[16px] p-5" style={{ background: '#EEF2F6', border: '1px solid rgba(31,58,95,0.15)' }}>
      ...
      <div className="mb-8 flex flex-wrap gap-2">
        {FILTER_TAGS.map(t => (
          <button key={t} onClick={() => setFilter(t)}
            ...
            style={filter === t ? { background: 'var(--brand-navy-900)', color: '#fff', borderColor: 'var(--brand-navy-900)' } : { background: 'white', color: '#334155', borderColor: '#E2E8F0' }}>
            {t}
          </button>
        ))}
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p, i) => (
            <button key={p.id || i} onClick={() => setSelected(p)}
              className="rounded-[20px] border border-[#E2E8F0] bg-white p-5 text-left transition hover:shadow-md hover:border-[rgba(31,58,95,0.25)] hover:-translate-y-0.5">
              <div className="mb-3 flex flex-wrap gap-1.5">
                {p.category?.slice(0, 2).map((c, ci) => (
                  <span key={ci} className="rounded-full px-2 py-0.5 text-xs font-semibold" style={{ background: '#EEF2F6', color: 'var(--brand-navy-900)' }}>{c}</span>
                ))}
