  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto" style={{ background: 'rgba(5,8,22,0.5)' }}>
      <div className="w-full max-w-2xl rounded-[24px] bg-white p-6">
        ...
        <p className="tp-prose mb-5 text-[color:var(--ink-500)]">All templates use clean, ATS-friendly one-column layouts. The Classic Finance template is recommended for finance, consulting, and traditional recruiting. No template guarantees employment or ATS approval.</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {TEMPLATES.map(t => (
            <button key={t.id} onClick={() => onSelect(t)}
              ...
              <p className="tp-card text-[color:var(--surface-dark-900)] group-hover:text-[color:var(--brand-navy-900)]">{t.name}</p>
              <p className="tp-meta mt-1 text-[color:var(--ink-500)]">{t.description}</p>
            </button>
          ))}
        </div>
      </div>
          ...
          <div className="rounded-xl border-2 p-3" style={{ borderColor: 'var(--brand-navy-700)', background: 'var(--ink-100)' }}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="tp-meta mt-1 text-[color:var(--ink-500)]">Last edited: {fmtDate(currentResume.updated_date)}</p>
              </div>
              <span className="tp-meta rounded-full px-2.5 py-0.5 font-bold text-white" style={{ background: 'var(--brand-navy-900)' }}>Live</span>
            </div>
          </div>
          {versions.length === 0 && (
            ...
            <div key={v.id} className="rounded-xl border border-[color:var(--ink-200)] bg-white p-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="tp-meta mt-1 text-[color:var(--ink-400)]">{fmtDate(v.created_date)}</p>
                </div>
                <button onClick={() => doRestore(v)} disabled={!!restoring}
                  className="tp-meta flex items-center gap-1.5 rounded-lg px-3.5 py-2 font-semibold border border-[color:var(--ink-200)] hover:border-[color:var(--brand-navy-900)] hover:text-[color:var(--brand-navy-900)] transition disabled:opacity-50">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(5,8,22,0.5)' }}>
      <div className="w-full max-w-md rounded-[24px] bg-white p-6">
        ...
        {status === 'idle' && (
          <>
            <p className="tp-prose mb-4 text-[color:var(--ink-700)]">
        ...
        {status === 'connecting' && <p className="tp-body py-6 text-center text-[color:var(--ink-500)]">Connecting to Google…</p>}
        {status === 'done' && (
          <div className="text-center">
            <Check size={32} className="mx-auto mb-3 text-green-600" />
            <p className="tp-card text-[color:var(--surface-dark-900)]">Google Doc created!</p>
            {docUrl && docUrl !== '#' && (
              <a href={docUrl} target="_blank" rel="noopener noreferrer"
                className="tp-body mt-3 inline-block underline" style={{ color: 'var(--brand-navy-700)' }}>Open Google Doc</a>
            )}
          </div>
        )}
        {status === 'error' && (
                ...
                <div key={r.id}
                  ...
                  <div className="flex items-start justify-between gap-1">
                    ...
                    <button onClick={e => { e.stopPropagation(); setDeleteTarget(r); }}
                      className="shrink-0 text-red-200 hover:text-red-500"><Trash2 size={13} /></button>
                  </div>
                  <p className="tp-meta mt-1 text-[color:var(--ink-400)]">{TEMPLATES.find(t => t.id === r.template_id)?.name || 'Custom'}</p>
                  {r.updated_date && <p className="tp-meta text-[color:var(--ink-400)]">{fmtDate(r.updated_date)}</p>}
                </div>
              ))}
            </div>
          )}
              ...
              <div className="ml-auto flex items-center gap-2 flex-wrap">
                ...
                <div className="flex rounded-xl border border-[color:var(--ink-200)] overflow-hidden">
                  ...
                  <button onClick={() => setView('preview')}
                    className={`tp-meta flex items-center gap-1.5 px-3.5 py-2.5 font-semibold transition ${view === 'preview' ? 'text-white' : 'text-[color:var(--ink-500)] hover:bg-[color:var(--ink-50)]'}`}
