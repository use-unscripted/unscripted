...
/**
 * The single dominant action for the student's current stage.
 * Deliberately the only high-emphasis control on the screen.
 */
export default function PrimaryActionCard({ action, pathName, onAnchorClick }) {
  const body = (
    <>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <Flame size={13} style={{ color: 'var(--brand-gold-500)' }} />
      ...
      {pathName && (
        <h2 className="font-heading mt-3 text-2xl font-bold text-white sm:text-3xl">{pathName}</h2>
      )}
