...
const field = 'w-full rounded-[var(--r-control)] border border-[color:var(--ink-200)] px-3 py-2 text-base md:text-sm text-[color:var(--ink-700)] outline-none focus:border-[color:var(--brand-navy-900)]';
...
export default function ResumeApprovalModal({ item, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4" style={{ background: 'rgba(5,8,22,0.5)' }}>
      <div className="anim-modal my-8 w-full max-w-lg rounded-[var(--r-surface)] bg-white p-6">
        ...
        <div className="my-4 flex gap-2 rounded-[var(--r-control)] p-3" style={{ background: 'var(--background-tertiary)' }}>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <button onClick={() => save('approved')} disabled={saving || !form.approved_title.trim()}
            className="flex flex-1 items-center justify-center gap-2 rounded-[var(--r-control)] py-2.5 text-sm font-semibold text-white disabled:opacity-50"
            style={{ background: 'var(--brand-navy-900)' }}>
            <Check size={14} /> {saving ? 'Saving…' : 'Approve for resume'}
          </button>
          <button onClick={() => save('excluded')} disabled={saving}
            className="rounded-[var(--r-control)] border border-[color:var(--ink-200)] px-4 py-2.5 text-sm font-semibold text-[color:var(--ink-700)] hover:bg-[color:var(--ink-50)]">
            Keep off my resume
          </button>
        </div>
