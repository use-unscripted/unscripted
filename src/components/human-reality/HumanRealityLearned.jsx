...
export default function HumanRealityLearned({ record, brief }) {
  ...
  return (
    <section className="app-card p-6 sm:p-8">
      ...
      <h2 className="tp-section mt-3" style={{ color: 'var(--ink-900)' }}>What this changed</h2>

      <dl className="mt-4 space-y-3">
        <div>
          <dt className="tp-label" style={{ color: 'var(--ink-500)' }}>Your key learning</dt>
          <dd className="tp-body mt-1" style={{ color: 'var(--ink-700)' }}>{record.key_learning}</dd>
        </div>
        <div>
          <dt className="tp-label" style={{ color: 'var(--ink-500)' }}>Your expectations</dt>
          <dd className="tp-body mt-1" style={{ color: 'var(--ink-700)' }}>{changed}</dd>
        </div>
        <div>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        <Link to="/matrix" className="ui-press app-cta tp-control">See it in your Decision Matrix</Link>
        <Link to="/journey" className="app-cta-secondary tp-control">Back to My Journey</Link>
      </div>
    </section>
  );
}