...
const inputCls = 'w-full rounded-[var(--r-control)] border border-[color:var(--ink-200)] bg-[color:var(--page-surface)] px-3 py-2.5 text-sm outline-none focus:border-[color:var(--brand-navy-900)]';
...
export default function ICSExportPanel({ showHeading = true }) {
  ...
  return (
    <div className="rounded-[var(--r-surface)] border border-[color:var(--ink-200)] bg-white p-6">
      {showHeading && (
        <>
      ...
      {error && (
        <div className="tp-meta mb-4 rounded-[var(--r-control)] border border-red-100 bg-red-50 px-4 py-3 text-red-700">{error}</div>
      )}

      <div className="space-y-3">
        {/* Week export */}
        <div className="rounded-[var(--r-surface)] border border-[color:var(--ink-200)] p-4">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-[var(--r-control)] flex items-center justify-center shrink-0" style={{ background: 'var(--ink-100)' }}>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <input type="date" value={weekStart} onChange={e => setWeekStart(e.target.value)}
                className="rounded-[var(--r-control)] border border-[color:var(--ink-200)] bg-[color:var(--page-surface)] px-3 py-2 text-base md:text-sm outline-none focus:border-[color:var(--brand-navy-900)]" />
              <button onClick={downloadWeek} disabled={downloading === 'week'}
                className="tp-meta touch-target flex items-center gap-1.5 rounded-[var(--r-control)] px-4 py-2.5 font-semibold text-white disabled:opacity-60"
