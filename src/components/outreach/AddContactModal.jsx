    ...
    <div role="alert" aria-live="polite"
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="tp-card text-[color:var(--surface-dark-900)]">Contact saved successfully.</p>
          <p className="tp-meta mt-1 truncate text-[color:var(--ink-500)]">{contact.name}</p>
          {experimentTitle && <p className="tp-meta truncate text-[color:var(--ink-400)]">Experiment: {experimentTitle}</p>}
          {missionTitle && <p className="tp-meta truncate text-[color:var(--ink-400)]">Mission: {missionTitle}</p>}
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(5,8,22,0.55)' }}>
      <div className="w-full max-w-2xl max-h-[92vh] overflow-y-auto rounded-[var(--r-surface)] bg-white p-6 sm:p-8">
        ...
        <p className="tp-lead mb-6 text-[color:var(--ink-500)]">{isEdit ? 'Update contact details and linked experiment.' : 'Connect this contact to an experiment and track outreach.'}</p>

        {saveError && <div className="tp-body mb-4 p-3 rounded-lg bg-red-50 text-red-700" role="alert">{saveError}</div>}

        {loadingData ? (
          <div className="tp-body py-10 text-center text-[color:var(--ink-500)]"><Loader2 size={20} className="animate-spin mx-auto mb-2" />Loading…</div>
        ) : (
          <div className="space-y-4">

            {/* Contact info */}
            <p className="tp-eyebrow text-[color:var(--ink-400)]">Contact Information</p>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">Full name <span className="text-red-500">*</span></label>
                <input name="name" value={data.name} onChange={ch} placeholder="Jane Smith" className={inputCls} />
                {errors.name && <p className="tp-meta mt-1.5 text-red-600">{errors.name}</p>}
              </div>
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">Company or organization</label>
                <input name="company" value={data.company} onChange={ch} placeholder="Goldman Sachs" className={inputCls} />
              </div>
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">Job title</label>
            ...
            {/* Contact methods */}
            <p className="tp-eyebrow pt-1 text-[color:var(--ink-400)]">Contact Methods <span className="normal-case font-normal">(at least one required)</span></p>
            {errors.contact_method && <p className="tp-meta -mt-2 text-red-600">{errors.contact_method}</p>}
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">Email</label>
                <input name="email" type="email" value={data.email} onChange={ch} placeholder="jane@company.com" className={inputCls} />
                {errors.email && <p className="tp-meta mt-1.5 text-red-600">{errors.email}</p>}
              </div>
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">Phone number</label>
                <input name="phone" type="tel" value={data.phone} onChange={ch} placeholder="(203) 555-0148 or +1 203 555 0148" className={inputCls} />
                {errors.phone && <p className="tp-meta mt-1.5 text-red-600">{errors.phone}</p>}
              </div>
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">LinkedIn URL</label>
                <input name="profile_url" value={data.profile_url} onChange={ch} placeholder="linkedin.com/in/janedoe" className={inputCls} />
                {errors.profile_url && <p className="tp-meta mt-1.5 text-red-600">{errors.profile_url}</p>}
              </div>
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">Website or portfolio</label>
                <input name="website_url" value={data.website_url} onChange={ch} placeholder="https://janedoe.com" className={inputCls} />
                {errors.website_url && <p className="tp-meta mt-1.5 text-red-600">{errors.website_url}</p>}
              </div>
            </div>

            {/* Experiment / Path / Mission */}
            <p className="tp-eyebrow pt-1 text-[color:var(--ink-400)]">Experiment &amp; Path</p>
            <div>
            ...
            {/* Outreach details */}
            <p className="tp-eyebrow pt-1 text-[color:var(--ink-400)]">Outreach Details</p>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
              ...
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">Last contacted</label>
                <input type="date" name="last_contacted_date" value={data.last_contacted_date} onChange={ch} className={inputCls} />
              </div>
              <div>
                <label className="tp-meta block mb-1.5 font-semibold text-[color:var(--ink-700)]">Follow-up date</label>
